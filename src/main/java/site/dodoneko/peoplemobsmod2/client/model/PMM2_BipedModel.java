...
public class PMM2_BipedModel<T extends Entity> extends EntityModel<T> implements IHasArm, IHasHead {
    ...
    public void render(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw,
        ...
        if (((LivingEntity) entityIn).getPrimaryHand() == HandSide.RIGHT) {
            ...
            } else if ((itemstack.getItem() instanceof net.minecraft.item.BlockItem)) {
                this.rightArmPose = ArmPose.BLOCK;
            } else if (!(itemstack.getItem() instanceof net.minecraft.item.AirItem)) {
                this.rightArmPose = ArmPose.ITEM;
            }
            if (itemstackOff.getItem() instanceof net.minecraft.item.BowItem) {
                this.leftArmPose = ArmPose.BOW_AND_ARROW;
            } else if ((itemstackOff.getItem() instanceof net.minecraft.item.BlockItem)) {
                this.leftArmPose = ArmPose.BLOCK;
            } else if (!(itemstackOff.getItem() instanceof net.minecraft.item.AirItem)) {
        ...
        if (this.doWalkBounding) {
            GlStateManager.translatef(0.0F,
                    MathHelper.abs(MathHelper.cos(limbSwing * 1.3314F)) * limbSwingAmount * this.modelScale * 0.0625F
                            * 4 - 0.0625F * 2 * limbSwingAmount * ((this.isChild || this.useChildModel) ? 0.5F : 1F),
                    0.0F);
        }
    ...
    public void setRotationAngles() {
        ...
        if (this.bipedLeftLeg.rotateAngleX < 0.0F) {
            ...
            // 0.523599rad '=, 30deg
            this.bipedLeftLeg.rotationPointY += gapLY = (float) Math
                    .sin(-Math.min(this.bipedLeftLeg.rotateAngleX, 0.523599D)) * 2.0F;
            this.bipedLeftLeg.rotationPointZ -= (1F
                    - (float) Math.cos(-Math.min(this.bipedLeftLeg.rotateAngleX, 0.523599D))) * 2.0F;
        }
        if (this.bipedRightLeg.rotateAngleX < 0.0F) {
            ...
            this.bipedRightLeg.rotationPointY += gapRY = (float) Math
                    .sin(-Math.min(this.bipedRightLeg.rotateAngleX, 0.523599D)) * 2.0F;
            this.bipedRightLeg.rotationPointZ -= (1F
                    - (float) Math.cos(-Math.min(this.bipedRightLeg.rotateAngleX, 0.523599D))) * 2.0F;
        }
    ...
    protected void setShippoAnimations() {
        if (this.shippoSwing) {
            ...
            if (this.doWalkBounding)
                this.bipedShippo.rotateAngleX += MathHelper.abs(MathHelper.cos(limbSwing * 1.3314F - 1.3F))
                        * this.limbSwingAmount * 0.8F;
        }
    }
    ...
    protected void setAhogeAnimations() {
        ...
        if (this.doWalkBounding)
            f1 += MathHelper.abs(MathHelper.cos(limbSwing * 1.3314F - 1.3F)) * this.limbSwingAmount;
        if (this.ahogeSwing)
            this.bipedAhoge.rotateAngleX = f1;
        if (this.kemomimiSwing)
            this.bipedKemomimi.rotateAngleX = f1 * 0.65F;
    }
    ...
    protected void setBoobsAnimations() {
        ...
        if (this.bipedUpperBoob.initRotateAngleX == 0.0F) {
            ...
            this.bipedUpperBoob.initRotateAngleX = this.bipedUpperBoob.rotateAngleX = -(float) Math
                    .asin((double) this.boobHeight * 0.707106781F) + 1.570796326F;
            float f = this.boobHeight;
            if (this.boobHeight > 0.5F)
                f = 1.0F - f;
            this.bipedLowerBoob.initRotateAngleX = this.bipedLowerBoob.rotateAngleX = -(this.bipedUpperBoob.rotateAngleX
                    - PMM2_Math.PI_Half) * (2 + f);
        }

        if (this.boobsSwing) {
            this.bipedUpperBoob.rotationPointY += Math.max(
                    Math.min(
                            (float) -(this.bipedBody.rotationPointY + this.entityIn.getMotion().y) * this.boobHeight
                                    * 1.5F,
                            1.0F),
                    -0.8F);
            this.bipedUpperBoob.rotateAngleX += (float) Math.max(
                    Math.min(
                            (float) -(this.bipedBody.rotationPointY + this.entityIn.getMotion().y) * this.boobHeight
                                    * 0.4F * Math.PI,
                            0.25F * Math.PI),
    ...
    protected void setSwimmingAnimations() {
        ...
        this.bipedLeftLeg.rotateAngleX = MathHelper.cos(this.ageInTicks * 0.6F + PMM2_Math.PI) * -0.5F + 0.2F
                + 0.4F * (1 - f1);
        ...
        this.bipedLeftArm.rotateAngleZ = MathHelper.cos(this.ageInTicks * 0.45F + PMM2_Math.PI) * (0.3F + 0.8F * f1) - 1
                - 0.3F * f1;
        ...
        this.bipedHead.rotationPointZ = this.bipedBody.rotationPointZ = -6F
                * (float) this.entityIn.getMotion().length();
    }
    ...
    protected void setLoveAnimations() {
        ...
        this.bipedBody.rotationPointX -= (float) (Math.sin(Math.sin(f1) * 0.157F) - Math.sin(Math.sin(f1) * 0.25F))
                * 8F;
        this.bipedHead.rotationPointX -= (float) (Math.sin(Math.sin(f1) * 0.157F) - Math.sin(Math.sin(f1) * 0.25F))
                * 8F;
    ...
    protected void setWalkingAnimations() {
        ...
        this.bipedRightArm.rotateAngleX = MathHelper.cos(this.limbSwing * 1.3324F + (float) Math.PI) * 2.0F
        ...
        this.bipedLeftLeg.rotateAngleX = MathHelper.cos(this.limbSwing * 1.3324F + (float) Math.PI) * 1.4F
                * this.limbSwingAmount;
        this.bipedRightLeg.rotationPointZ = MathHelper.sin(this.limbSwing * 1.3324F) * 1F * this.limbSwingAmount
                + 1F * this.limbSwingAmount;
        this.bipedLeftLeg.rotationPointZ = MathHelper.sin(this.limbSwing * 1.3324F + (float) Math.PI) * 1F
        ...
        if (this.doWalkBounding) {
            this.bipedBody.rotationPointY += MathHelper.abs(MathHelper.cos(limbSwing * 1.3314F)) * limbSwingAmount
                    * this.modelScale * 0.0625F * 4
                    - 0.0625F * 2 * limbSwingAmount * (this.isChild ? 0.5F : 1F) * this.scaleFactor;
            this.bipedHead.rotationPointY += MathHelper.abs(MathHelper.cos(limbSwing * 1.3314F)) * limbSwingAmount
                    * this.modelScale * 0.0625F * 4
                    - 0.0625F * 2 * limbSwingAmount * (this.isChild ? 0.5F : 1F) * this.scaleFactor;
        }
    }
    ...
    public void setModelAttributes(EntityModel<T> model) {
        ...
        if (model instanceof PMM2_BipedModel) {
            ...
            modelbiped.entityMotion = new Vec3d(modelbiped.entityIn.posX - modelbiped.entityIn.prevPosX,
                    modelbiped.entityIn.posY - modelbiped.entityIn.prevPosY,
                    modelbiped.entityIn.posZ - modelbiped.entityIn.prevPosZ);
